...
Entonces el propietario sería zeke y el grupo propietario sería paradyss. Si la anterior sentencia se hubiera ejecutado con sudo:

```bash
sudo usermod -g paradys zeke #ojo el grupo paradys debe haber sido creado
```

Tanto grupo propietario como usuario propietario sería root. En la práctica anterior vimos que:
...
2.Añade el grupo y crea contraseña de grupo (ejemplo `escuadrilla_A`) con `gpasswd`:
